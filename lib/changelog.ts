...
export const changelogItems: ChangelogItem[] = [
  {
    ...
    simplified: 'the swarm is less likely to get stuck when one internal connection blips. it can keep the important return path alive instead of forcing a full restart.',
  ...
  },
  {
    ...
    simplified: 'same GPUs, much faster engine. in the benchmark note this is roughly a ~3x tok/s jump overall, because local stage work drops from ~155ms to ~40ms on key cells.',
  ...
  },
  {
    ...
    simplified: 'better measurement and better ring planning. it separates what is slow: network, cpu, or stage compute.',
  ...
  },
  {
    ...
    simplified: 'image generation feels less broken while waiting. the UI keeps showing a live placeholder until the worker returns the final image.',
  ...
  },
  {
    ...
    simplified: 'workers no longer time out waiting on image jobs. the job can finish in the background and land when ready.',
  ...
  },
  {
    ...
    simplified: 'connects a swarm node identity to a c0mpute account. basically: know which GPU should get paid.',
